...
const URL = 'https://guymaich-jpg.github.io/Aravadistillery-Factory-Control/';
...
(async () => {
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      logResult('fail', `Page load at ${vp.name}`, err.message);
      await context.close();
      continue;
    }
    ...
    try {
      ...
      // Retry without fullPage if fonts cause timeout
      try {
        await page.screenshot({ path: ssPath, fullPage: false, timeout: 10000 });
        logResult('pass', `Screenshot saved (viewport only, fullPage timed out)`, ssPath);
      } catch (err2) {
        logResult('fail', `Screenshot failed`, err2.message.substring(0, 100));
      }
    }
    ...
    } catch (e) { /* page might be gone */ }
    if (hasHScroll) {
      const scrollDiff = await page.evaluate(() =>
        document.documentElement.scrollWidth - document.documentElement.clientWidth
      );
      logResult('fail', `Horizontal scrollbar detected`, `scrollWidth exceeds clientWidth by ${scrollDiff}px`);
    } else {
    ...
    let overflowIssues = [];
    try { overflowIssues = await page.evaluate(() => {
      const issues = [];
      const allElements = document.querySelectorAll('*');
      for (const el of allElements) {
        if (!el.offsetParent && el.tagName !== 'HTML' && el.tagName !== 'BODY') continue;
        const style = window.getComputedStyle(el);
        // Check for content overflowing its bounding box
    ...
    }); } catch (e) { logResult('warn', 'Could not check overflow', e.message.substring(0, 60)); }
    ...
    let overlaps = [];
    try { overlaps = await page.evaluate(() => {
    ...
    }); } catch (e) { logResult('warn', 'Could not check overlaps', e.message.substring(0, 60)); }
  ...
  // Use desktop viewport for accessibility checks
  ...
  try {
    ...
    console.log(`  FATAL: Could not load page for a11y tests: ${err.message}`);
    await browser.close();
    process.exit(1);
  }
  ...
  if (imageResults.length === 0) {
    logResult('pass', 'No <img> elements found (app uses icon SVGs/CSS instead)');
  } else {
    let imgIssues = 0;
  ...
  // Also check SVG elements for accessibility
  const svgResults = await page.evaluate(() => {
    ...
    for (const svg of svgs) {
      ...
      if (hasAriaHidden) withAriaHidden++;
      if (!hasTitle && !hasAriaLabel && !hasAriaHidden) {
        noA11y++;
        const parent = svg.parentElement;
        const parentInfo = parent ? `${parent.tagName.toLowerCase()}${parent.className ? '.' + String(parent.className).split(' ')[0] : ''}` : 'none';
        issues.push(parentInfo);
      }
    }
  ...
  if (svgResults.noA11y > 0) {
    ...
    logResult('pass', `All SVGs have accessibility attributes`);
  }
  ...
  const formResults = await page.evaluate(() => {
    ...
    for (const input of inputs) {
      ...
      if (!hasAssociatedLabel) {
        hasAssociatedLabel = input.closest('label') !== null;
      }
  ...
  if (formResults.length === 0) {
    logResult('pass', 'No form inputs found in current view (login may be JS-rendered)');
  } else {
    let unlabelled = 0;
    for (const input of formResults) {
      ...
      if (input.isLabelled) {
        ...
        logResult('fail', `${input.tag}[type=${input.type}] (id=${input.id}) has NO accessible label${hint}`);
      }
    }
    if (unlabelled > 0) {
      console.log(`\n  NOTE: ${unlabelled} form inputs lack proper labels. Placeholder text alone is NOT a valid label.`);
    }
  }
  ...
  const headingResults = await page.evaluate(() => {
    ...
    for (const h of headings) {
      const isVisible = h.offsetParent !== null ||
                        window.getComputedStyle(h).position === 'fixed' ||
                        h.closest('.sr-only') !== null;
      list.push({
        tag: h.tagName,
        level: parseInt(h.tagName.charAt(1)),
        text: h.textContent.trim().substring(0, 60),
        visible: isVisible,
        ariaHidden: h.getAttribute('aria-hidden') === 'true',
      });
    }
    return list;
  });
  ...
  if (headingResults.length === 0) {
    logResult('warn', 'No heading elements found in current view');
  } else {
    console.log(`\n  Heading hierarchy:`);
    let prevLevel = 0;
    ...
    if (h1Count > 1) {
      logResult('warn', `Multiple <h1> elements found (${h1Count})`, 'Best practice is to have a single h1');
    }
  }
  ...
  if (focusResults.length > 0) {
    ...
    if (focusInvisible === 0) {
      ...
      logResult('fail', `${focusInvisible} focusable elements lack visible focus indicators`);
    }
  }

  // Check for focus-visible CSS support
  ...
  if (hasFocusVisibleCSS === true) {
    logResult('pass', 'CSS includes :focus-visible styles');
  } else if (hasFocusVisibleCSS === 'focus-only') {
  ...
  if (contrastFails === 0) {
    logResult('pass', `All ${contrastResults.length} text elements pass WCAG AA contrast`);
  } else {
    console.log(`\n  Contrast summary: ${contrastFails} failures out of ${contrastResults.length} checked elements`);
  }
  ...
  const ariaResults = await page.evaluate(() => {
    ...
    return {
      ...
      ariaElementSample: ariaElements.slice(0, 15),
    };
  });
  ...
  if (Object.keys(ariaResults.ariaAttrs).length > 0) {
    ...
    logResult('pass', `ARIA attributes are being used (${ariaResults.totalAriaElements} elements)`);
  } else {
  ...
  if (ariaResults.roleElements.length > 0) {
    ...
    logResult('pass', `${ariaResults.roleElements.length} elements with explicit roles`);
  } else {
  ...
  if (ariaResults.landmarks.length > 0) {
    ...
    logResult('pass', `${ariaResults.landmarks.length} landmark regions defined`);
  } else {
    logResult('warn', 'No landmark regions (role="main", role="navigation", etc.) found');
  }
  ...
  console.log(`\n  Touch target summary: ${touchResults.passing}/${touchResults.total} pass minimum 44x44px`);

  if (touchResults.tooSmall.length > 0) {
    logResult('fail', `${touchResults.tooSmall.length} interactive elements below 44x44px minimum`);
  ...
  if (liveRegions.length > 0) {
    ...
    logResult('warn', 'No aria-live regions found — dynamic content changes may not be announced to screen readers');
  }
  ...
  if (viewport) {
    logResult('pass', 'Meta viewport tag present', viewport);
  ...
  const hasReducedMotion = await page.evaluate(() => {
    ...
    for (const sheet of sheets) {
      try {
        for (const rule of sheet.cssRules) {
          if (rule instanceof CSSMediaRule && rule.conditionText && rule.conditionText.includes('prefers-reduced-motion')) {
            return true;
          }
        }
  ...
  if (hasReducedMotion) {
    ...
    logResult('warn', 'No prefers-reduced-motion support detected');
  }

  // Check prefers-color-scheme / dark mode support
  ...
  if (badTabindex.length > 0) {
    logResult('warn', `${badTabindex.length} elements with tabindex > 0 (disrupts natural tab order)`);
